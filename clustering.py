...
clustering_data = merged_df[['Risk (Std Dev)', 'Expected Return', 'ESG Score']].dropna()

# Apply KMeans clustering
kmeans = KMeans(n_clusters=3, random_state=42)
clustering_data['Cluster'] = kmeans.fit_predict(clustering_data)

# Merge clustering results back to the original DataFrame
merged_df = merged_df.merge(clustering_data[['Risk (Std Dev)', 'Expected Return', 'ESG Score', 'Cluster']],
